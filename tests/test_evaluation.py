...
def test_run_evaluation_matches_topic_equivalent_document_ids(tmp_path: Path) -> None:
    artifact_dir = tmp_path / "rag"
    dataset = tmp_path / "questions.jsonl"
    chunks = [
        Chunk(
    ...
    documents = [
        {
            "source": "osu-wiki",
            "page_id": "Beatmap/HP_drain_rate",
            "title": "HP drain rate",
            "repo_rel_path": "Beatmap/HP_drain_rate/en.md",
            "sections": [{"title": "HP drain rate", "heading_path": ["HP drain rate"]}],
        }
    ]
    write_jsonl(artifact_dir / "chunks_hierarchical.jsonl", chunks)
    write_jsonl(artifact_dir / "documents_structured.jsonl", documents)
    dataset.write_text(
        '{"category": "gameplay", "question": "What is Beatmap Health drain?", '
        '"expected_document_ids": ["Beatmap/Health_drain"]}\n',
        encoding="utf-8",
    )
    config = AppConfig(artifacts=ArtifactConfig(path=artifact_dir), retrieval=RetrievalConfig(final_top_k=1))
    ...
    assert report["examples"][0]["topic_match"] == 1
    assert report["examples"][0]["top_sources"][0]["topic_id"] == "Beatmap/Health_drain"
